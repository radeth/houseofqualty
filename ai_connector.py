...
def get_req_parm_matrix_from_ai(product_name, product_description, client_requirements, technical_parameters):
    client_requirements_text = ', '.join(item['name'] for item in client_requirements)
    technical_parameters_text = ', '.join(item['name'] for item in technical_parameters)
    prompt = f"Product Name: {product_name} Product Description: {product_description} Client Requirements: {client_requirements_text} Technical Parameters: {technical_parameters_text}"
        ...
        messages=[
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "Jesteś ekspertem w budowaniu dom jakosci qfd. **Zbuduj Matrycę:** Skonstruuj matrycę zależności, umieszczając parametry techniczne na jednej osi, a wymagania klientów na drugiej. **Wykorzystaj Nazwę, Opis Produktu:** Zastosuj dostarczoną nazwę i opis produktu, aby lepiej zrozumieć kontekst i wyodrębnić istotne parametry techniczne oraz wymagania klientów. **Określ Zależności:** Zanalizuj, w jaki sposób każdy parametr techniczny wpływa na każde wymaganie klienta. Użyj skali, np. 0 (brak. zależności), 1 (słaba zależność), 3(średnia zależność), 9 (silna zależność). Skoncentruj się na dokładnym określeniu stopnia wpływu, aby matryca była jak najbardziej użyteczna."
                    }
                ]
            },
            {
                    ...
                    "properties": {
                        ...
                        "dependency_matrix": {
                            ...
                            "items": {
                                ...
                                "description": "A single row of dependencies corresponding to a client requirement."
                            },
                            "description": "A matrix indicating the dependency between client requirements and technical parameters."
                        },
                        "technical_parameters": {
                            "type": "array",
                            "items": {
